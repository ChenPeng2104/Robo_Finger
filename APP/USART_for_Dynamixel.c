...
/**************************************************************/
/** @name   USART2_Init(int baud_rate)
  * @brief  USART2初始化   开接收中断 PA2-TX PA3-RX
  * @param  None
  * @retval None
  */
/**************************************************************/
...
Dynamixel_Structure *Receive_Cmd_TTL(void)
{
    ...
    if( (100+Receive_Que.rear-Receive_Que.front)%100>5  )
    {
        if( ((Receive_Que.Data[(Receive_Que.front+1)%100])==0xff)&&(Receive_Que.Data[(Receive_Que.front+2)%100]==250))    //正常数据的开头，检测到两个0XFF,和0xfd  判断第三个是否是250
        {
            //确认完整的接收完一数据包(暂时不检查是否传输数据有错)
//            Receive_Que_TTL.Data[(Receive_Que_TTL.front+2)%100]=250;
            receive_TTL_dynl.ID = Receive_Que.Data[(Receive_Que.front+2)%100];   	//ID
            receive_TTL_dynl.Len =  Receive_Que.Data[(Receive_Que.front+3)%100];	//长度字节
            for(i=0; i<receive_TTL_dynl.Len; i++)
            {
                receive_TTL_dynl.Data[i]=Receive_Que.Data[(Receive_Que.front+4+i)%100];
            }
            Receive_Que.front = (Receive_Que.front+6)%100;
            return &receive_TTL_dynl;
        }
        else  //虽然检测到OXFF。但是并不是 0XFF 0XFF 0XFD 所以跳过
        {
